...
public class RealmsSliderButton extends RealmsButton {
    ...
    public void renderBg(int p_renderBg_1_, int p_renderBg_2_) {
        if (this.getProxy().field_146125_m) {
            if (this.sliding) {
                this.value = (float) (p_renderBg_1_ - (this.getProxy().x + 4)) / (float) (this.getProxy().func_146117_b() - 8);
                if (this.value < 0.0F) {
                    this.value = 0.0F;
                }
            ...
            this.blit(this.getProxy().x + (int) (this.value * (float) (this.getProxy().func_146117_b() - 8)), this.getProxy().y, 0, 66, 4, 20);
            this.blit(this.getProxy().x + (int) (this.value * (float) (this.getProxy().func_146117_b() - 8)) + 4, this.getProxy().y, 196, 66, 4, 20);
        }
    }

    public void clicked(int p_clicked_1_, int p_clicked_2_) {
        this.value = (float) (p_clicked_1_ - (this.getProxy().x + 4)) / (float) (this.getProxy().func_146117_b() - 8);
        if (this.value < 0.0F) {
            this.value = 0.0F;
        }
